...
class TensLinker():
    ...
    def parse_link(self):
        ...
        dirty_link = urllib.request.urlopen('{}'.format(self.user_link)).read()   # Читаем ссылку
        ...
        dirty_text = str(text_title) + '\n' + str(text_news)  # соединяем заголовок и текст
    ...
    def text_writer(self, clean_text):
        ...
        os.makedirs(re.sub(r'http\S?://(www.)?', '', self.user_link))  # Создаем нужные папки (с одноименным названием)
        os.chdir(re.sub(r'http\S?://(www.)?', '', self.user_link))  # Переходим в новую директорию
